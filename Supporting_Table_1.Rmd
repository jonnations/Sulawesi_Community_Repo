...
vdat <- 
  ...
  select(!contains("ses")) %>% 
  select(!c(all_var, vN15, vC13, iv_sum, iv_scaled, iso_var)) %>% 
  group_by(com) %>% 
...
vdf <- bind_cols(nam, trt, df2) %>% 
  ...
  mutate(Trait = recode(Trait, 
                        ...
                        'iso_var_group' = "Isospace" ,
                        'morpho_var' = "Morphology",
                        'vBsz' = "Body Size",
                        'vC13_group' = "Carbon13",
                        'vN15_group' = 'Nitrogen15',
...
tabl %>% write_csv("Table_S1_output1.csv")
```
...
```{r}
tabl <- read_csv("Table_S1_output2.csv") %>%
  mutate_if(is.numeric, ~round(., 2)) %>% 
