...
Making a big table of the median and 89% CIs of the values and SES's of the Variance and Density of each trait.

# Load Packages and Models
```{r}
pacman::p_load(here,tidyverse,tidybayes, kableExtra)
...
tabl %>% write_csv(here("Table", "Table_S1_temp.csv"))
```
...
This writes a .csv that serves as Table S2

```{r}
tabl <- read_csv(here("Table", "Table_S1_temp2.csv")) %>%
  mutate_if(is.numeric, ~round(., 2)) %>% 
  mutate_at(vars(3:6), ~ifelse(. == 'NaN (NA, NA)', NA, .)) %>% 
  drop_na(Volume) %>% 
  select(!comb) %>% write_csv(here("Table", "Table_S2.csv"))
```
