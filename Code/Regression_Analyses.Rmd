...
## Load Up
```{r}
pacman::p_load(tidyverse, tidybayes, brms, modelr, purrr, cmdstanr, patchwork)
...
dat <- read_csv("All_Traits.csv") %>% select(-.draw) %>% select(contains(c("ses", "com")))
...
write_rds(ttt, "Models/All_Regression_Models_List2.rds")
```

### Cleanup
```{r}
ttt <- readRDS("Models/All_Regression_Models_List2.rds")
...
```{r}
all_fin <- bind_rows(fin, iso_fin) %>% write_csv("Regression_Results2.csv")
...
```{r}
locdat <- read_csv("Locomotion_Data/All_Locomotor_var.csv") 
```
...
l.1 <- brm(data = locdat, 
           ...
           cores = 4, 
           refresh = 0) 
l.2 <- update(l.1, formula = ses_loc_prob ~ nsp, newdata = locdat, refresh = 0)
...
```{r}
phydat <- read_csv("Phylogenetic_Diversity/PD_Results.csv") %>% rename(nspec = ntaxa)
phydat2 <- read_csv("Phylogenetic_Diversity/PD_Results_no_Hae.csv") %>% rename(nspec = ntaxa)
...
p3 <- bind_cols((cleanup(m3) %>% rename(b_lower = "Q5.5",
                       b_upper = "Q94.5")),
          (cleanupI(m3)%>% rename(I_lower = "Q5.5",
                       I_upper = "Q94.5")),
(cleanupS(m3)%>% rename(s_lower = "Q5.5",
                       s_upper = "Q94.5")),
(cleanupR(m3)%>% rename(R2_lower = "Q5.5",
                       R2_upper = "Q94.5")),
(cleanupRsm(m3)%>% rename(R2sm_lower = "Q12.5",
                       R2sm_upper = "Q87.5"))) %>% add_column(var = "vphy2")
          
p4 <- bind_cols((cleanup(m4) %>% rename(b_lower = "Q5.5",
                       b_upper = "Q94.5")),
          (cleanupI(m4)%>% rename(I_lower = "Q5.5",
                       I_upper = "Q94.5")),
(cleanupS(m4)%>% rename(s_lower = "Q5.5",
                       s_upper = "Q94.5")),
(cleanupR(m4)%>% rename(R2_lower = "Q5.5",
                       R2_upper = "Q94.5")),
(cleanupRsm(m4)%>% rename(R2sm_lower = "Q12.5",
...
```{r}
all_fin %>%  write_csv("Regression_Results.csv")
```
