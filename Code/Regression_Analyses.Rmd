...
Does species richness have an effect on ecospace volume or density? These regression analyses will help to answer this question. This is a series of Bayesian regression analyses using `brms`. The input data are n_species as a predictor, the (sum of) variance(s) and density as responses, but the responses INCLUDE a parameter of the standard error of each functional space metric. This greatly improves the models as it effectively integrates uncertainty into the models, contributing a lot of information that would otherwise be missing. 

This only uses the SES estimates, not the raw volume and density values.

## Load Up
```{r}
pacman::p_load(here, tidyverse, tidybayes, brms, modelr, purrr, cmdstanr, patchwork)
...
```
## SE function
Get mean and standard error of each value for each community. The measurement error models require standard error.
...
isodat <- dat %>% select(c(com, ses_vIso, ses_vC13, ses_vN15, nnses_C13, nnses_N15, nnses_iso)) %>% filter(com %in% keepcoms)

adat <- dat %>% select(!c(ses_vIso, ses_vC13, ses_vN15, nnses_C13, nnses_N15, nnses_iso)) 
...
Now collect the traits that have 9 communities of data (non-iso traits)
```{r}
mycols <- adat %>% select(-com) %>% colnames()
```

### Generate nested dataset
All the data is split by the trait name into a nested dataframe. 
...
Results in a nice dataframe with beta and 89% intervals and Intercept and 89% intervals
...
nest_data <- nest_data %>% mutate(fits = future_map(d, ~update(fit, newdata = .x, refresh = 0)))

plan(sequential)

write_rds(nest_data, here("Models", "All_Regression_Models_List2.Rds"))
```

# Cleanup Model Outputs
```{r}
nest_data <- read_rds(here("Models", "All_Regression_Models_List2.Rds"))

b <- nest_data %>%  mutate(beta = map(fits, cleanup)) %>%
                unnest(beta) %>% 
                rename(b_lower = "Q5.5",
                       b_upper = "Q94.5") %>% 
                select(-c(d, fits))
i <- nest_data %>%  mutate(int = map(fits, cleanupI)) %>% 
                unnest(int) %>%
                rename(I_lower = "Q5.5",
                       I_upper = "Q94.5") %>%
                ungroup() %>% 
                select(-c(d, fits, var))
s <- nest_data %>%  mutate(sig = map(fits, cleanupS)) %>% 
                unnest(sig) %>%
                rename(s_lower = "Q5.5",
                       s_upper = "Q94.5") %>%
                ungroup() %>% 
                select(-c(d, fits, var))
r <- nest_data %>%  mutate(r2 = map(fits, cleanupR)) %>% 
                unnest(r2) %>%
...
rsm <- nest_data %>%  mutate(r2sm = map(fits, cleanupRsm)) %>% 
                unnest(r2sm) %>%
...
Do the same thing, but for the Isotope Data
...
```{r}
all_fin <- bind_rows(fin, iso_fin) %>% write_csv(here("Regression", "Regression_Results2.csv"))
...
l.3 <- update(l.1, formula = nn_rda ~ nsp, newdata = locdat, refresh = 0)
l.4 <- update(l.1, formula = nnses_loc ~ nsp, newdata = locdat, refresh = 0)
```
...
(cleanupRsm(l.3)%>% rename(R2sm_lower = "Q12.5",
                       R2sm_upper = "Q87.5"))) %>% add_column(var = "nn_rda")
...
(cleanupRsm(l.4)%>% rename(R2sm_lower = "Q12.5",
                       R2sm_upper = "Q87.5"))) %>% add_column(var = "nn_ses_loc")
...
```{r}
all_fin %>%  write_csv(here("Regression", "Regression_Results.csv"))
```
