...
This is using the variance Using PCs 1-36.
```{r}
# Parallel computing options
furrr_options(seed = TRUE)
options(future.rng.onMisuse="ignore")
...
```{r}
myx <- "Mean Nearest Neighbor Distance SES"

p1 <- adat %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x =nnses_c, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="PC's 1-2", y = "", fill = "Mountain") +
  xlab(bquote(.(myx))) +
  xlim(-3, 3) + 
...
p2 <- adat %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x =nnses_c36, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
...
p1 <- adat %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x =nnses_d, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="PC's 1-2", y = "", fill = "Mountain") +
  xlab(bquote(.(myx))) +
  xlim(-3, 3) + 
...
p2 <- adat %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x =nnses_d20, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
...
nn_sk_rand2 <- bind_rows(skamb, skbaw, skbul, skdak, skgan, skkat, sklat, sknok, sktor) 
rm(skamb, skbaw, skbul, skdak, skgan, skkat, sklat, sknok, sktor)
```

### Skull Shape 56 SES
```{r}
adat <- nn_sk_rand2 %>% group_by(com) %>% 
  summarise(mean = mean(value),
            sd = sd(value)) %>% 
  as.data.frame() %>% 
  right_join(adat, by="com") %>%
