...
```{r}
pacman::p_load(here, tidyverse, tidybayes, patchwork, modelr, ggstar)
...
i_plot3 <- read_csv(here("All_Traits.csv")) %>% select(com, ses_vIso_group) %>% drop_na() %>% 
...
e_plot3 <- read_csv(here("All_Traits.csv")) %>% select(com, ses_vTail) %>% drop_na() %>% 
  mutate(com = factor(com, level = level_order)) %>% 
  mutate(com = recode(com,
                      'Ambang' = 'Ambang (13)',
                      'Bawakaraeng' = 'Bawakaraeng (7)',
...
ggsave(here("Plots", "Fig5.png"), height = 13, width =9)
