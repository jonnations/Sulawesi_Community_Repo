...
extdat <- read_csv(here("External_Measurement_Data", "Measurement_Data_Museum.csv")) %>% 
  ...
  group_by(Species) %>% 
  summarise_at(vars(HB, lMass:rTail), list(m = mean, sd = sd)) %>% 
  ungroup()
...
Here I didn't group by draw so that I could calculate means and SDs for each community on the whole. These values are used for plotting points with error bars.
```{r}
...
ggplot(aes(y = com, x = mHB)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
  # Add points
  geom_point(aes(x = HB_m, y = com), data = extdat) +
  #labs(title="Cranial Shape Position",x="PC1 Mean", y = "", fill = "Mountain") +
  geom_vline(xintercept = 0, linetype = 2) +
  theme_bw() +
  scale_fill_paletteer_d('ggsci::default_gsea', direction = 1) +
...
ggplot(aes( x = mTail,y = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
  # Add points
  geom_point(aes(x = rTail_m, y = com), data = extdat) +
  geom_vline(xintercept = 0, linetype = 2) +
  theme_bw() +
  scale_fill_paletteer_d('ggsci::default_gsea', direction = 1) +
  guides(fill = guide_legend(reverse=T)) + theme(legend.position = "none")
...
### WIth Convex Hulls rather than low-alpha dots

This won't work unless I go back to the previous one.
```{r}
#convex hulls
hull <- ext %>% 
  group_by(com) %>% 
  slice(chull(mTail, mHB)) %>% 
  mutate(com = factor(com, level = level_order))
...
ggplot(aes( x = mTail, y = mHB, color = com)) +
  ...
  geom_polygon(aes(fill = com, 
               ...
               size = 0.2) +
  # Add points
#  geom_jitter(aes(x = rTail_m, y = HB_m, size = #lMass_m, color = com), data = extdat, width = 0.09, #height = 0.09, shape = 5) +
#  scale_size(range = c(0.5, 5)) +
  geom_point(size = 8) +
  #labs(title="Cranial Shape Position",x="PC1 Mean", y = "", fill = "Mountain") +
  theme_bw() +
...
ggplot(aes( x = mTail, y = mHB, color = com)) +
  #geom_density_2d(bins =5, contour_var = "ndensity") +
  # Add points
  geom_jitter(aes(x = rTail_m, y = HB_m, size = lMass_m), data = extdat, width = 0.09, height = 0.09, shape = 5) +
  scale_size(range = c(0.5, 5)) +
  geom_point(size = 6) +
  geom_errorbar(aes(ymin = mHB - (sdHB/2) ,
                    ymax = mHB + (sdHB/2))) + 
  geom_errorbarh(aes(xmin = mTail - (sdTail/2),
                     xmax = mTail + (sdTail/2))) +
  #labs(title="Cranial Shape Position",x="PC1 Mean", y = "", fill = "Mountain") +
  theme_bw() +
  scale_fill_paletteer_d('ggsci::default_gsea', direction = 1) +
