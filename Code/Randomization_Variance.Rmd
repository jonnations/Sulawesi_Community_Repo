...
Warning:: this may take hours depending on furrr & future settings. Using `workers = 8` on an Apple M1 takes about 4 minutes per community, or 36 min total.
```{r}
plan(multisession, workers = 8)
tictoc::tic()
...
## Cranial Randomization 2 Axes

### Cranium 2 Variance Function  
similar to the last one...
```{r}
get_cvar <- function(sp, data, mountain) {
  data %>% 
  ...
  split(.$.draw) %>% 
  map(., ~ .x[ ,-c(1:2)]) %>% 
  map(~ .x %>% summarise(across(cPC1:cPC2, ~ var(.x, na.rm = TRUE)))) %>% 
    bind_rows() %>% 
    as_tibble() %>%
      mutate(vc = select(., cPC1:cPC2) %>% rowSums(na.rm = TRUE)) %>% 
  sample_n(., size = 100) %>% 
    add_column(com = {{mountain}})
}
```
...
c_rand2 <- bind_rows(camb, cbaw, cbul, cdak, cgan, ckat, clat, cnok, ctor) 
rm(camb, cbaw, cbul, cdak, cgan, ckat, clat, cnok, ctor)
tictoc::toc()
```
...
### Cranium 2 SES
```{r}
adat <- c_rand2 %>% group_by(com) %>% 
  summarise(vmean = mean(vc),
            vsd = sd(vc)) %>% 
  as.data.frame() %>% 
  right_join(adat, by="com") %>%
  mutate(ses_cv_wei2 =  (cv_wei2 - vmean) / vsd) %>% 
  select(!c(vmean, vsd))
...
## Dentary Randomization 3 Axes

### Dentary 3 Randomization Function
```{r}
get_dvar <- function(sp, data, mountain) {
  data %>% 
    filter(Species %in% {{sp}}) %>% 
  split(.$.draw) %>% 
  map(., ~ .x[ ,-c(1:2)]) %>% 
  map(~ .x %>% summarise(across(dPC1:dPC3, ~ var(.x, na.rm = TRUE)))) %>% 
    bind_rows() %>% 
    as_tibble() %>%
    mutate(vd = select(., dPC1:dPC3) %>% rowSums(na.rm = TRUE)) %>%
    select(vd) %>%
    sample_n(., size = 100) %>% 
    add_column(com = {{mountain}})
}
...
d_rand2 <- bind_rows(damb, dbaw, dbul, ddak, dgan, dkat, dlat, dnok, dtor) 
rm(damb, dbaw, dbul, ddak, dgan, dkat, dlat, dnok, dtor)
tictoc::toc()
```
...
### Dentry 3 SES
```{r}
adat <- d_rand2 %>% group_by(com) %>% 
  summarise(mean_d = mean(vd),
            sd_d = sd(vd)) %>% 
  as.data.frame() %>% 
  right_join(adat, by="com") %>%
  mutate(ses_dv_wei3 =  (dv_wei3 - mean_d) / sd_d) %>% 
  select(!c(mean_d, sd_d))
```
...
get_skvar <- function(sp, data, mountain) {
  data %>% 
    ...
    split(.$.draw) %>% 
    map(., ~ .x[ ,-c(1:2)]) %>% 
    map(~ .x %>% summarise(across(c(cPC1:cPC2, dPC1:dPC3), ~ var(.x, na.rm = TRUE)))) %>% 
    bind_rows() %>% 
    as_tibble() %>%
    mutate(vc = select(., cPC1:cPC2) %>% rowSums(na.rm = TRUE),
           vd = select(., dPC1:dPC3) %>% rowSums(na.rm = TRUE),
           vsk5 = (vd + vc) / 2) %>%  
      select(vsk5) %>%
  sample_n(., size = 100) %>% 
    add_column(com = {{mountain}})
...
sk_rand2 <- bind_rows(skamb, skbaw, skbul, skdak, skgan, skkat, sklat, sknok, sktor) 
rm(skamb, skbaw, skbul, skdak, skgan, skkat, sklat, sknok, sktor)
tictoc::toc()
```
...
adat <- sk_rand2 %>% group_by(com) %>% 
  summarise(mean = mean(vsk5),
            sd = sd(vsk5)) %>% 
  as.data.frame() %>% 
  right_join(adat, by="com") %>%
  mutate(ses_vsk5 =  (((cv_wei2 + dv_wei3)/2) - mean) / sd) %>% 
  select(!c(mean, sd))
...
### Isotope Estimate simulated communities  
