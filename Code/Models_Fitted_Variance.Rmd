...
```{r}
pacman::p_load(here, tidyverse, brms, tidybayes, patchwork, modelr, cmdstanr, paletteer, qs)
...
qsave(mvcPC2, here("Models","Cranium_36_PC_fitted.qs"))
```
...
qsave(mvdPC2, here("Models", "Dentary_20_PC_fitted.qs"))
``` 
...
qsave(m.ex1, here("Models", "External_Measurement_Fitted.qs"))
```
...
data %>% 
  ...
  summarise(vHB = var(HB),
            ...
            mHB_pos = mean(HB),
            mTail_pos = mean(rTail),
            mHF_pos = mean(rHF),
            mEar_pos = mean(rEar),
            mMass_pos = mean(lMass)) %>% 
...
qsave(m.iso, here("Models", "iso_fitted_Com_Grouping.qs"))
```
...
ggplot(aes(y = com, x = vN15, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="Tail Length",x="Trait Volume", y = "", fill = "Mountain") +
  theme_bw() + 
  scale_fill_paletteer_d('ggsci::default_gsea', direction = 1)  +
