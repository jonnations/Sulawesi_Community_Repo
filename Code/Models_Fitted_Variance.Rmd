...
```{r}
pacman::p_load(here, tidyverse, brms, tidybayes, patchwork, modelr, cmdstanr, paletteer)
...
ggplot(aes(y = com, x = cv_wei, fill = com)) +
...
ggplot(aes(y = com, x = dv_wei, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
...
ggplot(aes(y = com, x = vTail, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
...
```
###########
#NOW THAT I KNOW BAWA IS THE LOWEST MEAN TAIL LENGTH, I WOULD NEED TO GO BACK UP AND SUBSTRACT THE... WAIT, NOT SURE. 
...
ggplot(aes(y = com, x = vN15_group, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="Tail Length",x="Trait Volume", y = "", fill = "Mountain") +
  theme_bw() + 
  scale_fill_paletteer_d('ggsci::default_gsea', direction = 1)  +
...
#### Diversity Index (Shannon, or H')
Trying this instead of variance, which doesn't make sense with categorical data. 
```{r}
locdiv <- read_csv(here("Locomotion_Data", "Locomotion_Div_Data.csv")) 

div <- locdiv %>% select(!com) %>% 
  diversity(., "shannon")

locdiv2 = locdiv %>% add_column(div) %>% write_csv(here("Locomotion_Data", "All_Locomotor_diversity.csv"))
```

#### Quick Plot
```{r}
locdiv2 %>% mutate(com = factor(com, level = level_order),
                   n = (A + C + T + SA)) %>% 
ggplot(aes(y = n, x = div, color = com)) +
  #stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
  geom_point(size = 8) +
   labs(title="Locomotor Variance",x="Locomotor Diversity", y = "Richness") +
  theme_bw() + 
  scale_color_paletteer_d('ggsci::default_gsea', direction = 1)  +
  xlim(0.4, 1.1) +
  guides(fill = guide_legend(reverse=T)) + theme(legend.position = "none")
```
