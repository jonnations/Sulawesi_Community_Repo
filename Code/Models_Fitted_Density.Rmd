...
nn_dent2 <- bind_rows(amb_nni, baw_nni, bul_nni, dak_nni, gan_nni, kat_nni, lat_nni, nok_nni, tor_nni) %>% rename(d_nn20 = value)
```
...
p1 <- nn_dent %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x =d_nn, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="PC's 1-3", y = "", fill = "Mountain") +
  xlab(bquote(.(myx))) +
  #xlim(1, 3.2) + 
  theme_bw() + scale_fill_viridis_d(direction = -1, begin = 0, end = 0.9, option = "A") + guides(fill = guide_legend(reverse=T)) + theme(legend.position = "none") 
