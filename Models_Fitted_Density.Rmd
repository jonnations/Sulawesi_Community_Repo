...
```{r}
pacman::p_load(tidyverse, tidybayes, brms, modelr, furrr, dispRity)
...
nn_csize %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x =1/csize_nn, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="Cranial size", y = "", fill = "Mountain") +
  xlab(bquote(.(myx)^-1)) +
  xlim(1.25, 9.5) +
  theme_bw() + scale_fill_viridis_d(direction = -1, begin = 0, end = 0.9,  option = "A")  + guides(fill = guide_legend(reverse=T)) + theme(legend.position = "none") 
```
...
adat <- adat %>% select(!d_nn) %>% bind_cols(dd)
...
nn_dent %>% 
  filter(d_nn >= 0.3) %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x =1/d_nn, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="Broken Stick: PC's 1-3", y = "", fill = "Mountain") +
  xlab(bquote(.(myx)^-1)) +
  theme_bw() + scale_fill_viridis_d(direction = -1, begin = 0, end = 0.9, option = "A") + guides(fill = guide_legend(reverse=T)) + theme(legend.position = "none") 
```
...
adat <- adat %>% select(!dsize_nn) %>% bind_cols(dd)
#adat %>% write_csv("CLEAN_All_Traits.csv")
```
...
nn_dsize %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x = 1/dsize_nn, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="Dentary size",x="Mean Nearest Neighbor Distance", y = "", fill = "Mountain") +
     labs(title="Dentary Centroid Size", y = "", fill = "Mountain") +
  xlim(1.5, 11.2) +
  xlab(bquote(.(myx)^-1)) +
  theme_bw() + scale_fill_viridis_d(direction = -1, begin = 0, end = 0.9, option ="A") + guides(fill = guide_legend(reverse=T)) + theme(legend.position = "none") 
```
...
# Add Skull Results to DF
Add these updated results to the clean traits df
```{r}
#all_var <- read_csv("CLEAN_All_Traits.csv") %>%
  select(!c(c_nn, d_nn, csize_nn, dsize_nn))   

all_var <- 
  all_var %>% 
  bind_cols(nn_cran[,1]) %>% 
  bind_cols(nn_dent[,1]) %>% 
  bind_cols(nn_csize[,1]) %>%
  bind_cols(nn_dsize[,1]) 
#write_csv(all_var, "CLEAN_All_Traits.csv")
```
...
#### Add Ext Results to DF
Add these to the clean df
```{r}
all_var <- 
  all_var %>% 
  bind_cols(nn_Ear[,1]) %>% 
...
## Iso Group
```{r}
idat <- read_csv("Isotope_Data/Isotope_Group_Level_Fitted.csv", col_select = c(Species, .draw, sC13, sN15))
...
# Isotope Nearest Neighbor 
Not Using THis

#### Set Up fitted values
This will go just like the others, but I first need to scale all values by community, then run a model, get the fitted values, then proceed as usual. This is one way of accounting for the background variation in the communities isotopic values
```{r}
idat <- read_csv("Isotope_Data/Isotope_Data.csv") %>% 
  # group by location, scale by location, then ungroup
  group_by(Location) %>% 
  mutate(sbN15 = scale2(Corrected_15N),
         sbC13 = scale2(Corrected_13C)) %>% 
  ungroup()

imod <- bf(sbN15 ~ 0 + (1|Species)) + 
          bf(sbC13 ~ 0 + (1|Species))  +
          set_rescor(FALSE)
...
m.iso <- brm(imod,
          ...
          control = list(adapt_delta = .99,
                         max_treedepth = 12),
          file = "Models/iso_fitted_scaled_by_com")

pre.iso <- idat %>%
  data_grid(Species) %>%
  add_fitted_draws(m.iso) %>% 
  spread(.category, .value) %>% 
  select(!c(.chain, .iteration))

pre.iso %>% write_csv("Isotope_Data/Isotope_Scale_by_Location_Fitted.csv")
```
...
```{r}
isof <- pre.iso %>% 
  select(c(Species, .draw, sbN15)) 

amb_nni <- get_mnni(isof, spamb) %>%  mutate(com = "Ambang" )
baw_nni <- get_mnni(isof, spbaw) %>%  mutate(com = "Bawakaraeng" )
bul_nni <- get_mnni(isof, spbul) %>%  mutate(com = "Buliohuto" )
dak_nni <- get_mnni(isof, spdak) %>%  mutate(com = "Dako" )
gan_nni <- get_mnni(isof, spgan) %>%  mutate(com = "Gandangdewata" )
lat_nni <- get_mnni(isof, splat) %>%  mutate(com = "Latimojong" )

nn_N15 <- bind_rows(amb_nni, baw_nni, bul_nni, dak_nni, gan_nni, lat_nni) %>% rename(N15_nn = value)

nn_N15 %>% 
  mutate(com = factor(com, level = level_order)) %>% 
 ggplot(aes(y = factor(com, level = level_order), x =N15_nn, fill = com)) +
  stat_halfeye(.width = c(0.74, 0.89),  alpha = 0.7) +
   labs(title="N15 community scaled",x="Mean Nearest Neighbor Distance", y = "", fill = "Mountain") +
  theme_bw() + scale_fill_viridis_d(direction = -1, begin = 0, end = 0.95) + guides(fill = guide_legend(reverse=T)) + theme(legend.position = "none") 
```

## C13 NN
```{r}
isof <- pre.iso %>% 
  select(c(Species, .draw, sbC13)) 
...
nn_C13 <- bind_rows(amb_nni, baw_nni, bul_nni, dak_nni, gan_nni, lat_nni) %>% rename(C13_nn = value)
```

## Iso NN
```{r}
isof <- pre.iso %>% 
  select(c(Species, .draw, sbC13, sbN15)) 
...
nn_iso <- bind_rows(amb_nni, baw_nni, bul_nni, dak_nni, gan_nni, lat_nni) %>% rename(iso_nn = value)
```

#Add Iso Results to DF

```{r}
adat <- read_csv("CLEAN_All_Traits.csv") %>% select(!c(N15_nn, C13_nn, iso_nn))

  iso_nn <- 
  nn_N15 %>% 
  bind_cols(nn_C13[,1]) %>% 
  bind_cols(nn_iso[,1]) 
iso_nn$.draw = rep(1:4000, 6)

adat <- adat %>%  
  full_join(iso_nn, by = c(".draw", "com"))

#write_csv(adat, "CLEAN_All_Traits.csv")
```
