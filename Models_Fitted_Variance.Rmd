...
```{r}
pdat <- read_csv("Cranial_Data/Cranial_PCA_Data_36axes.csv") %>% 
...
mvcPC2<- brm(mvcmod,
          ...
          file = "Models/Cranium_36_PC_fitted")
```
...
write_csv(pre.cranium_t, "Cranial_Data/Cranial_36axes_Fitted.csv")
```
...
write_csv(cct, "Cranial_Data/Community_Cranial_Variance_Weighted.csv")
```
...
```{r}
ddat <- read_csv("Dentary_Data/Dentary_PCA_Data_20axes.csv") %>% 
  mutate_if(is.numeric, scale2, na.rm = TRUE)
...
mvdPC2<- brm(mvdmod,
          ...
          warmup = 4000,
          refresh = 0,
          file = "Models/Dentary_20_PC_fitted")
``` 
...
write_csv(pre.dentary_t, "Dentary_Data/Dentary_20axes_Fitted.csv")
```
...
write_csv(dct, "Dentary_Data/Community_Dental_Variance_Weighted.csv")
```
...
```{r}
dat <- read_csv("External_Measurement_Data/Measurement_Data_Museum.csv") %>% 
...
m.ex1<- brm(exmod,
          ...
          refresh = 0,
          cores = 4,
          file = "Models/External_Measurement_Fitted")
```
...
write_csv(ext_fit , "External_Measurement_Data/External_Fitted.csv")
```
...
rm(dambt, dbawt, dbult, ddakt, dgant, dkatt, dlatt, dnokt, dtort)
write_csv(ext, "External_Measurement_Data/Community_External_Variance_Fitted.csv")
```
...
m.iso <- brm(imod,
          ...
          control = list(adapt_delta = .99,
                         max_treedepth = 12),
          file = "Models/iso_fitted_Com_Grouping"
          )
...
write_csv(iso_fit , "Isotope_Data/Isotope_Group_Level_Fitted.csv")
...
write_csv(ivar, "Isotope_Data/Isotope_Group_Level_Variance.csv")
```
...
Read in Data
```{r}
cdat <- read_csv("Cranial_Data/Community_Cranial_Variance_Weighted.csv")
ddat <- read_csv("Dentary_Data/Community_Dental_Variance_Weighted.csv")
edat <- read_csv("External_Measurement_Data/Community_External_Variance_Fitted.csv")
idat <- read_csv("Isotope_Data/Isotope_Group_Level_Variance.csv")
...
write_csv(all_var, "All_Traits_Variance_Fitted.csv")
```
...
write_csv(vLoc, "Locomotion_Data/All_Locomotor_var.csv")
```
