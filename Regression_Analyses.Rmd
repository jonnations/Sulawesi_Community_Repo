...
isodat <- dat %>% select(c(com, ses_vIso_group, ses_vC13_group, ses_vN15_group, nnses_C13_group, nnses_N15_group, nnses_iso_group)) %>% filter(com %in% keepcoms)

adat <- dat %>% select(!c(ses_vIso_group, ses_vC13_group, ses_vN15_group, nnses_C13_group, nnses_N15_group, nnses_iso_group)) 
...
write_rds(ttt, "Models/All_Regression_Models_List.rds")
```

### Cleanup
```{r}
ttt <- readRDS("Models/All_Regression_Models_List.rds")
...
```{r}
all_fin <- bind_rows(fin, iso_fin) %>% write_csv("Regression_Results.csv")
...
all_fin <- bind_rows(p1, p2, all_fin)
```
