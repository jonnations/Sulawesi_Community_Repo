...
iso.fit <- read_csv("Isotope_Data/Isotope_Group_Level_Fitted.csv")
...
iso_fit_n <- iso_fit %>% add_column(Community = "Species Mean") %>% mutate(.epred = sN15) %>% as_tibble() %>%  select(c(Community, Species, .epred)) 
...
```{r}
iso_fit_c <- iso_fit %>% add_column(Community = "Species Mean") %>% mutate(.epred = sC13) %>% as_tibble() %>%  select(c(Community, Species, .epred)) 
