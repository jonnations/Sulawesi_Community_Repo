---
title: "ALL REGRESSION PLOTS"
---

# Plotting for FIg 3

This csv I am reading in contains some updated fields that I added manually. These describe the traits or trait types, and are there for organizing. All numeric values are the same as the regression outputs.
```{r}
pacman::p_load(tidyverse, tidybayes, tictoc, brms, modelr, wesanderson, patchwork)
fin <- read_csv("Fig3_Regression_Plot_Data.csv") %>% drop_na()
```


## Plot SES Var

```{r}
layout <- "
AAB
CCD
CCD
"

p1 + p2 + p3 + p4 + plot_layout(design = layout, guides = "collect", tag_level = 'new') + plot_annotation(tag_levels = list(c('A', '', 'B', '')))

ggsave("V_SES_Regression.pdf", width = 6, height = 10, useDingbats = F)
```
#### Same plot magma
```{r}
p1 <- fin %>% 
  filter(type == "v_ses",
         ntraits == "multi") %>% 
    arrange(Beta) %>%  
 ggplot(aes(y = factor(name, levels=name), x = Beta, xmin = b_lower, xmax = b_upper, color = Beta)) +
  #scale_color_manual(values = cols) +
  scale_color_viridis_c(direction = -1, begin = 0.17, end = 0.71, option = "B") +
  geom_vline(xintercept = 0, linetype = 2 ) + 
  geom_pointrange(size = 1.5, show.legend = F) +
  labs(x = expression(beta), y="") +
  xlim(-.13, .33) +
  ggtitle("Ecospace Variance") +
  theme_classic() 

p2 <- fin %>% 
  filter(type == "v_ses",
         ntraits == "multi") %>% 
    arrange(Beta)  %>% ggplot(aes(y = factor(name, levels=name), x = R2sm, xmin = R2sm_lower, xmax = R2sm_upper#, color = Effect_Size
                            )) +
  scale_color_manual(values = cols) +
  #geom_vline(xintercept = 0, linetype = 2 ) +
  geom_pointrange(size = 1.5) +
  labs(x = bquote("Bayesian"~ R^2), y = "") +
  xlim(0,1) +
  #ggtitle("Trait Space SES - Total Species Pool") +
  theme_classic() +
  theme(axis.text.y=element_blank()) 



p3 <- fin %>% 
  filter(type == "v_ses",
         ntraits == "one") %>% 
    arrange(Beta) %>%  
 ggplot(aes(y = factor(name, levels=name), x = Beta, xmin = b_lower, xmax = b_upper, color = Beta)) +
  #scale_color_manual(values = cols) +
  scale_color_viridis_c(direction = -1,  option = "B", end = 0.9) +
  geom_vline(xintercept = 0, linetype = 2 ) + 
  geom_pointrange(size = 1.5, show.legend = F) +
  xlim(-.13, .33) +
  labs(x = expression(beta), y="") +
  ggtitle("Individual Trait Variance") +
  theme_classic() 

p4 <- fin %>% 
  filter(type == "v_ses",
         ntraits == "one") %>% 
    arrange(Beta)  %>% ggplot(aes(y = factor(name, levels=name), x = R2sm, xmin = R2sm_lower, xmax = R2sm_upper#, color = Effect_Size
                            )) +
  scale_color_manual(values = cols) +
  #geom_vline(xintercept = 0, linetype = 2 ) +
  geom_pointrange(size = 1.5) +
  labs(x = bquote("Bayesian"~ R^2), y = "") +
  xlim(0,1) + 
  #ggtitle("Trait Space SES - Total Species Pool") +
  theme_classic() +
  theme(axis.text.y=element_blank()) 


layout <- "
AAB
CCD
CCD
"

p1 + p2 + p3 + p4 + plot_layout(design = layout, guides = "collect", tag_level = 'new') + plot_annotation(tag_levels = list(c('A', '', 'B', '')))

ggsave("Plots/Fig3A_V_SES_Regression_Magma.pdf", width = 6, height = 10, useDingbats = F)
```

# Plot NN SES

```{r}
p1 <- fin %>% 
  filter(type == "nn_ses",
         ntraits == "multi") %>% 
    arrange(Beta) %>%  
 ggplot(aes(y = factor(name, levels=name), x = Beta, xmin = b_lower, xmax = b_upper, color = Beta)) +
  #scale_color_manual(values = cols) +
  scale_color_viridis_c(direction = -1, begin = 0.17, end = 0.82, option = "B") +
  geom_vline(xintercept = 0, linetype = 2 ) + 
  geom_pointrange(size = 1.5, show.legend = F) +
  labs(x = expression(beta), y="") +
  xlim(-.23, .43) +
  ggtitle("Ecospace NN Distance") +
  theme_classic()

p2 <- fin %>% 
  filter(type == "nn_ses",
         ntraits == "multi") %>% 
    arrange(Beta)  %>% ggplot(aes(y = factor(name, levels=name), x = R2sm, xmin = R2sm_lower, xmax = R2sm_upper#, color = Effect_Size
                            )) +
  scale_color_manual(values = cols) +
  #geom_vline(xintercept = 0, linetype = 2 ) +
  geom_pointrange(size = 1.5) +
  labs(x = bquote("Bayesian"~ R^2), y = "") +
  xlim(0,1) +
  #ggtitle("Trait Space SES - Total Species Pool") +
  theme_classic() +
  theme(axis.text.y=element_blank()) 



p3 <- fin %>% 
  filter(type == "nn_ses",
         ntraits == "one") %>% 
    arrange(Beta) %>%  
 ggplot(aes(y = factor(name, levels=name), x = Beta, xmin = b_lower, xmax = b_upper, color = Beta)) +
  #scale_color_manual(values = cols) +
  scale_color_viridis_c(direction = -1, option = "B", end = 0.9) +
  geom_vline(xintercept = 0, linetype = 2 ) + 
  geom_pointrange(size = 1.5, show.legend = F) +
  xlim(-.23, .43) +
  labs(x = expression(beta), y="") +
  ggtitle("Individual Trait NN Distance") +
  theme_classic() 

p4 <- fin %>% 
  filter(type == "nn_ses",
         ntraits == "one") %>% 
    arrange(Beta)  %>% ggplot(aes(y = factor(name, levels=name), x = R2sm, xmin = R2sm_lower, xmax = R2sm_upper#, color = Effect_Size
                            )) +
  scale_color_manual(values = cols) +
  #geom_vline(xintercept = 0, linetype = 2 ) +
  geom_pointrange(size = 1.5) +
  labs(x = bquote("Bayesian"~ R^2), y = "") +
  xlim(0,1) + 
  #ggtitle("Trait Space SES - Total Species Pool") +
  theme_classic() +
  theme(axis.text.y=element_blank()) 

layout <- "
AAB
CCD
CCD
"

p1 + p2 + p3 + p4 + plot_layout(design = layout, guides = "collect", tag_level = 'new') + plot_annotation(tag_levels = list(c('C', '', 'D', '')))

ggsave("Plots/Fig3B_NN_SES_Regression_Magma.pdf", width = 6, height = 10, useDingbats = F)
```

