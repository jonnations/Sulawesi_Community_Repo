...
get_cvar <- function(sp, data, mountain) {
  ...
  map(~ .x %>%
    ...
    bind_rows() %>% 
    as_tibble() %>%
    mutate(vc = vcPC1 + vcPC2 + vcPC3 + vcPC4 + vcPC5 + vcPC6 + vcPC7 + vcPC8 + vcPC9 + vcPC10 + vcPC11 + vcPC12 + vcPC13 + vcPC14 + vcPC15 + vcPC16 + vcPC17 + vcPC18 + vcPC19 + vcPC20 + vcPC21 + vcPC22 + vcPC23 + vcPC24 + vcPC25 + vcPC26 + vcPC27 + vcPC28 + vcPC29 + vcPC30 + vcPC31 + vcPC32 + vcPC33 + vcPC34 + vcPC35 + vcPC36) %>%  
      select(vc, vcCsize) %>%
  sample_n(., size = 100) %>% 
    add_column(com = {{mountain}})
}
...
```{r}
adat5 %>% write_csv("All_Traits.csv")
```
...
adat6 %>% write_csv("All_Traits.csv")
